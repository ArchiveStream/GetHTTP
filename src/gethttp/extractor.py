...
class VideoExtractor():
    ...
    def p_stream(self, stream_id):
        ...
        if 'itag' in stream:
            print("    # download-with: %s" % log.sprint("gethttp --itag=%s [URL]" % stream_id, log.UNDERLINE))
        else:
            print("    # download-with: %s" % log.sprint("gethttp --format=%s [URL]" % stream_id, log.UNDERLINE))
